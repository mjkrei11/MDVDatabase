...
CREATE procedure [dbo].[sp_ssrs_ReputationQA_Specialties_Archive](
...
exec sp_ssrs_ReputationQA_Specialties_Archive @Database, @CompOption, @RatingsSite, @YearQuarter, @option
*/
...
set @sql = @sql + 'select media.CollectionID, media.CollectionName, media.SystemID, media.SystemName, media.NPI, media.FirstName, media.MiddleName, media.LastName, ' + @CR
...
set @sql = @sql + 'and media.NPI like ''S%'' '
exec(@sql)
