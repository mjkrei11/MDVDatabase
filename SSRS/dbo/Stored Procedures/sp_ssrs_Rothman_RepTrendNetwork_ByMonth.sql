...
CREATE procedure [dbo].[sp_ssrs_Rothman_RepTrendNetwork_ByMonth](
...
exec sp_ssrs_Rothman_RepTrendNetwork_ByMonth @Database, @StartMonth, @EndMonth
*/
...
set @sql = @sql + 'avg(isnull(metric.RepTrendRating, 0.0)) as RepTrendRating, sum(isnull(metric.RepTrendCount, 0)) as RepTrendCount ' + @CR
...
/* --This Section will display the Top 10 for each Site
select		distinct Top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, s.RatingSite,
			isnull(s.Rating, 0.00) as StartingRating, isnull(s.Volume, 0) as StartingVolume, 
			isnull(e.Rating, 0.00) as EndingRating, isnull(e.Volume, 0) as EndingVolume, 
			isnull(e.Rating, 0) - isnull(s.Rating, 0) as RatingDiff, isnull(e.Volume, 0) - isnull(s.Volume, 0) as VolumeDiff
from		#start_values s
left join	#end_values e
on			e.NPI = s.NPI
and			e.RatingSite = s.RatingSite
where		s.RatingSite = 'HealthGrades' and e.RatingSite = 'HealthGrades'
--order by	RatingDiff desc, s.PhysicianName
union
select		distinct Top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, s.RatingSite,
			isnull(s.Rating, 0.00) as StartingRating, isnull(s.Volume, 0) as StartingVolume, 
			isnull(e.Rating, 0.00) as EndingRating, isnull(e.Volume, 0) as EndingVolume, 
...
on			e.NPI = s.NPI
and			e.RatingSite = s.RatingSite
where		s.RatingSite = 'RateMDs' and e.RatingSite = 'RateMDs'
--order by	RatingDiff desc, s.PhysicianName
union
select		distinct Top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, s.RatingSite,
			isnull(s.Rating, 0.00) as StartingRating, isnull(s.Volume, 0) as StartingVolume, 
...
on			e.NPI = s.NPI
and			e.RatingSite = s.RatingSite
where		s.RatingSite = 'Vitals' and e.RatingSite = 'Vitals'
--order by	RatingDiff desc, s.PhysicianName
union
select		distinct Top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, s.RatingSite,
			isnull(s.Rating, 0.00) as StartingRating, isnull(s.Volume, 0) as StartingVolume, 
...
on			e.NPI = s.NPI
and			e.RatingSite = s.RatingSite
where		s.RatingSite = 'UCompare' and e.RatingSite = 'UCompare'
and			e.NPI <> '1649324195' and s.NPI <> '1649324195'
order by	s.RatingSite, VolumeDiff asc
*/
