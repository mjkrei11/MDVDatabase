...
CREATE procedure [dbo].[sp_ssrs_Rothman_RepTrendNetwork_TopRating_ByMonth](
...
exec sp_ssrs_Rothman_RepTrendNetwork_TopRating_ByMonth @Database, @StartMonth, @EndMonth
*/
...
create table #TopRating(TopRatingStartDate date, TopRatingEndDate date, TopRatingPhysicianName nvarchar(400), TopRatingStartingRating float, TopRatingStartingVolume int, 
			TopRatingEndingRating float, TopRatingEndingVolume int, TopRatingRatingDiff float, TopRatingVolumeDiff int, DataType nvarchar(50))
insert #TopRating
select		distinct Top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, 
			...
			round(isnull(round(e.Rating,2), 0) - isnull(round(s.Rating,2), 0),2) as RatingDiff, isnull(e.Volume, 0) - isnull(s.Volume, 0) as VolumeDiff, 'Top Rating' as DataType
