...
CREATE procedure [dbo].[sp_SSRS_AppUpdate_PracticeRatingUpdate] (
...
exec sp_SSRS_AppUpdate_PracticeRatingUpdate @Database
*/
...
set @sql = @sql + 'select distinct NPI, RatingsSite, CardProgramRating * CardProgramVolume ' + @CR
...
set @sql = @sql + 'select distinct NPI, sum(CardProgramVolume) ' + @CR
set @sql = @sql + 'from ' + @Database + '.dbo.PhysicianRatingUpdate ' + @CR
Set @sql = @sql + 'WHERE convert(varchar, UpdatedDate, 101) = ''' + convert(varchar, @date, 101) + ''' ' + @CR
...
set @sql = @sql + 'sum(CardProgramVolume) AS AggregateBaselineVolume, u.Score AS AggregateUpdatedRating, ' + @CR
...
set @sql = @sql + 'sum(cast(UpdatedVolume as int)) - sum(cast(CardProgramVolume as int)) AS AggregateVolumeDifference, p.UpdatedDate ' + @CR
