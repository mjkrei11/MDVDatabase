...
CREATE procedure [dbo].[sp_ssrs_ReferralData](
...
exec sp_ssrs_ReferralData @Database, @CompetitionOption, @NPIList, @SpecialtyWhere, @CityWhere, @StateWhere, @ReferralCountWhere, @UniqueBeneficiariesWhere, @SameDayReferralsWhere, @ReferringNPIList
*/
...
begin
	...
	begin
		...
		set @sql = @sql + 'select distinct NPI, isnull(PrimaryCustomerSpecialty, PrimarySpecialty), CustomerName ' + @CR
		set @sql = @sql + 'from MDVALUATE.dbo.MasterPhysicianMedia ' + @CR
		set @sql = @sql + 'where TreeLevelI = ''Competition'' ' + @CR
		set @sql = @sql + 'and TreeLevelII = ''' + @CustomerSource + ''' ' + @CR
		set @sql = @sql + 'and Status = ''Active'' ' + @CR
		exec(@sql)
	...
	set @sql = @sql + 'from CMS_Referrals.dbo.vw_CMSReferralData v ' + @CR
