...
CREATE procedure [dbo].[sp_ssrs_Rothman_RepTrendNetwork_NegativeComments_ByMonth](
...
exec sp_ssrs_Rothman_RepTrendNetwork_NegativeComments_ByMonth @Database, @StartMonth, @EndMonth
*/
...
set @sql = @sql + 'and metric.RepCommentIsNegative = ''1'' ' + @CR
...
create table #NegComments(NegCommentStartDate date, NegCommentEndDate date, NegCommentPhysicianName nvarchar(400), --RatingSites nvarchar(50),
			NegCommentBeginCount int, NegCommentEndCount int, NegCommentVolumeDiff int, DataType nvarchar(50))
insert #NegComments
select		distinct top 10 s.ValuesDate as StartDate, e.ValuesDate as EndDate, s.PhysicianName, --s.RatingSite,
			case when (s.CommentCount IS NULL) then '0' else s.CommentCount end as StartCommentCount, e.CommentCount as EndCommentCount,
			isnull(e.CommentCount, 0) - isnull(s.CommentCount, 0) as CommentDiff, 'Top Negative Comments' as DataType
from		#start_values s
...
create table #NegCommentSum(NegCommentStartDate date, NegCommentEndDate date, NegCommentPhysicianName nvarchar(400),
			NegCommentBeginCount int, NegCommentEndCount int, NegCommentVolumeDiff int, DataType nvarchar(50))
insert #NegCommentSum
select NegCommentStartDate, NegCommentEndDate, NegCommentPhysicianName, sum(NegCommentBeginCount) as NegCommentBeginCount,
sum(NegCommentEndCount) as NegCommentEndCount, sum(NegCommentEndCount) - sum(NegCommentBeginCount) as NegCommentVolumeDiff, DataType from #NegComments
group by NegCommentStartDate, NegCommentEndDate, NegCommentPhysicianName, DataType
order by NegCommentVolumeDiff desc

select * from #NegCommentSum where NegCommentEndCount > 0 order by NegCommentEndCount desc

drop table #NegCommentSum
