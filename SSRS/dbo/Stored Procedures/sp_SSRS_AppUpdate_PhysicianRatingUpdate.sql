...
CREATE procedure [dbo].[sp_SSRS_AppUpdate_PhysicianRatingUpdate] (
...
exec sp_SSRS_AppUpdate_PhysicianRatingUpdate
*/
...
----This has the CardProgram data as the baseline 
...
Set @sql = @sql + 'select u.NPI, u.FirstName, u.LastName, u.FullName, u.RatingsSite, ''5/6/2015'', u.CardProgramRating, u.CardProgramVolume, u.CardProgramDate, u.RatingDifference, u.VolumeDifference, ' + @CR
...
Set @sql = @sql + '(select distinct s.UpdatedRating from ' + @Database + '.dbo.PhysicianRatingUpdate s where s.NPI = u.NPI and s.RatingsSite = u.RatingsSite and s.UpdatedDate = u.UpdatedDate ), ' + @CR
...
--Set @sql = @sql + 'WHERE convert(varchar, u.UpdatedDate, 101) <> ''2015-07-09%'' '  + @CR    
Set @sql = @sql + 'union ' + @CR
Set @sql = @sql + 'select u.NPI, u.FirstName, u.LastName,  u.FullName, u.RatingsSite, ''9/14/2015'', u.UpdatedRating, u.UpdatedVolume, u.UpdatedDate, u.RatingDifference, u.VolumeDifference, ' + @CR
Set @sql = @sql + 'cast(d.BinData as varbinary(max)), ' + @CR
Set @sql = @sql + '(select distinct s.BaselineRating from ' + @Database + '.dbo.PhysicianRatingUpdate s where s.NPI = u.NPI and s.RatingsSite = u.RatingsSite and s.UpdatedDate = u.UpdatedDate ), ' + @CR
Set @sql = @sql + '(select distinct s.BaselineVolume from ' + @Database + '.dbo.PhysicianRatingUpdate s where s.NPI = u.NPI and s.RatingsSite = u.RatingsSite and s.UpdatedDate = u.UpdatedDate ), ' + @CR
Set @sql = @sql + '(select distinct s.BaselineDate from ' + @Database + '.dbo.PhysicianRatingUpdate s where s.NPI = u.NPI and s.RatingsSite = u.RatingsSite and s.UpdatedDate = u.UpdatedDate ), ' + @CR
